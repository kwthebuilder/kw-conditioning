...
const files: Record<string, unknown> = {
  ...
  'engine_test_vectors_v1_3.json': vectorsJson,
};
...
describe('spec loader', () => {
  it('loads and validates all three files', () => {
    ...
    expect(TEST_VECTORS.version).toBe('1.3');
    expect(INITIAL_STATE.schema).toBe(1);
    expect(CONFIG_VERSION).toBe('1.3');
    expect(VECTORS_VERSION).toBe('1.3');
  ...
  it('returns values structurally identical to the files (nothing dropped or coerced)', () => {
    ...
    expect(TEST_VECTORS).toEqual(raw('engine_test_vectors_v1_3.json'));
  });
...
describe('validators reject malformed input with a JSON path', () => {
  ...
  const vectors = (): Record<string, unknown> => structuredClone(raw('engine_test_vectors_v1_3.json')) as Record<string, unknown>;
